...
public:
  GLCanvas(QWidget * parent):QGLWidget(QGLFormat(QGL::SampleBuffers), parent),
    m_initialized(0),
  ...
  void postprocess()
  {
    m_width =((QWidget*)parent())->width()-4;
    m_height=((QWidget*)parent())->height()-4;
    m_verts = cvCloneMat(CV_DATA_WHOLE_BRAIN_SURF_verts());
    m_faces = cvCloneMat(CV_DATA_WHOLE_BRAIN_SURF_faces());
    cvAddS(m_faces,cvScalar(-1),m_faces);
    if (m_faces&&m_verts&&(!initialized())){
      initialize();
      resizeEvent(new QResizeEvent(QSize(m_width,m_height),QSize(width(),height())));
      updateGL();
    }
  }
#endif
